  ...
  <body>
  	<div class="container-fluid">
	  	<div class="row">
  			...
  			<div class="col-sm-12" style="margin-top:20px;">
			 	<table id="portList" class="table table-condensed table-hover table-striped" ajax="true">
				    <thead>
				        <tr>
				...
				<div class="modal-content">
				      <div class="modal-header" >
				      	<button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button>
		       			<h4 class='modal-title' id='myModalLabel'>添加/修改端口封堵信息</h4>
				      </div>
				      <div class="modal-body">
				      	<form id='changeport' class='form-horizontal' role='form'>
						    <div class='row'>
							  <div class='form-group'>
							    <label  class='col-sm-3 control-label'>名称*:</label>
							    <div class='col-sm-5' id='name_alert'  data-toggle='popover' data-trigger='manual'>
									<input type='text' name='portGroupPageModel.name'   class='form-control' id='name'/>
								</div>
							  </div>
							  <div class='form-group'>
							    <label  class='col-sm-3 control-label'>备注:</label>
							    <div class='col-sm-5' id='remark_alert'  data-toggle='popover' data-trigger='manual'>
									<input type='text'  name='portGroupPageModel.remark'   class='form-control' id='remark'/>
							    </div>
							  </div>
							  <div class='form-group' id='addinput'>
							    <div class='col-sm-offset-3 col-sm-5' >
							    	<button type="button" class='col-sm-12 btn btn-info' id="addport">添加端口</button>   
							  	</div>
							  </div>
							  <div id="portgroup"> 
							  </div>
	...
	<script type="text/javascript">
		...
		$("#portList").bootgrid({
	    	ajax: true,
	        post: function (){},
	        url: "${ctx}/json/portGroupAction!findByPage.action",
	        selection: true,
	        multiSelect: true,
	        rowSelect:true,
	        rowCount:10,
		...
		$("#addport").click(function(){
			var html="<div class='form-group' id='itemdiv_"+index+"'>"+
						"<div class='col-sm-5 col-sm-offset-3' id='item"+index+"_alert'  data-toggle='popover' data-trigger='manual'>"+
							"<input type='text'  name='item' placeholder='只能输入数字'  class='form-control' id='item"+index+"' onfocus='hiddenalert("+index+")'/>"+
						"</div>"+
		...
		$("#updateButton").click(function(){
			if(rowIds.length==1){
				...
				$("#portgroup").empty();
				index=1;
				$.post('${ctx}/json/portGroupAction!findById.action',{'id':rowIds[0]}, function(data) {
					$("#name").val(data.portGroupPageModel.name);
					oldname= data.portGroupPageModel.name;
					$("#remark").val(data.portGroupPageModel.remark);
					var items=data.portGroupPageModel.items;
					if(null!=items && ""!=items){
						...
						for(var i=1;i<=item.length;i++){
							var html="<div class='form-group' id='itemdiv_"+i+"'>"+
								"<div class='col-sm-5 col-sm-offset-3' id='item"+i+"_alert'  data-toggle='popover' data-trigger='manual'>"+
									"<input type='text'  name='portGroupPageModel.item'   class='form-control' value='"+item[i-1]+"' id='item"+i+"' onfocus='hiddenalert("+i+")'/>"+
								"</div>"+
		...
		function save(){
			...
			for(var i=1;i<=index-1;i++){
				var item=$("#item"+i).val();
				if(item==""){
					$("#item"+i+"_alert").attr("data-content","端口不能为空");
					$("#item"+i+"_alert").popover('show');
					return;
				}
				if(item.length>30){
					$("#item"+i+"_alert").attr("data-content","端口长度不能超过30");
					$("#item"+i+"_alert").popover('show');
					return;
				}
				if(!/\d/.test(item)){
					$("#item"+i+"_alert").attr("data-content","端口只能是数字");
					$("#item"+i+"_alert").popover('show');
					return;
				}
				items+=item+",";
			...
			$.post('${ctx}/json/portGroupAction!checkNameExist.action',{'name':name},function(datamap){
				if(savaState=="新增"){
					if(checkSubmitFlg==true){ 
						...
						if(datamap.success==true){
							...
							$.ajax({
									type:"POST",
									url:"${ctx}/json/portGroupAction!save.action?portGroupPageModel.items="+items,
									data:$("#changeport").serialize(),
									success:function(data){
										if(data.success==true){
											$('#infoModal').modal('hide');
											$("#portList").bootgrid("reload");
					...
					if(datamap.success==false||oldname==name){	
						$.ajax({
							type:"POST",
							url:"${ctx}/json/portGroupAction!update.action?portGroupPageModel.id="+rowIds[0]+"&&portGroupPageModel.items="+items,
							data:$("#changeport").serialize(),
							success:function(data){
								if(data.success==true){
									$('#infoModal').modal('hide');
		...
		$("#delButton").click(function(){
			...
			$.confirm({
    		    ...
    		    content: '你确定要删除这些数据吗？',
    		    confirm: function(){
    		    	$.post('${ctx}/json/portGroupAction!delete.action',{'ids':rowIds.toString()},function(data){
						 if(data.success==true){
							 $("#portList").bootgrid("reload");
							 rowIds=[];
							 $.messager.show({title:'提示信息',msg:data.msg});
