...
function login() {
	...
	if (a == "") {
		var c = "请输入邮箱/手机号/昵称";
		$("#modal-login-name").parent().append(
				"<label class='lblerr'>" + c + "</label>");
		d = false
	}
	if (b == "") {
		var c = "请输入密码";
		$("#modal-login-pwd").parent().append(
				"<label class='lblerr'>" + c + "</label>");
		d = false
	}
	if (d) {
...
function reg() {
	...
	if (a == "") {
		var c = "请输入邮箱/手机号";
		$("#modal-reg-name").parent().append(
				"<label class='lblerr'>" + c + "</label>");
		d = false
	}
	if (b == "") {
		var c = "请输入密码";
		$("#modal-reg-pwd").parent().append(
				"<label class='lblerr'>" + c + "</label>");
		d = false
	} else {
		if (b.length < 6) {
							...
							if (f && f.r == 1) {
								if (f.hasSendMail == 1) {
									...
									if (f.loginAddr && f.loginAddr != "") {
										e += "<p style='text-align:center;'><a href='"
												+ f.loginAddr
												+ "' target='_blank' rel='nofollow' role='button' class='btn' id='btn-tomymail'>前往邮箱查收</a></p>"
									}
				...
				.post(
						...
						function(b) {
							if (b.r != 1) {
								$("#reg-captcha")
										.append(
												"<label class='lblerr' style='right:120px'>验证码出错</label>");
								$("#captcha-a").click();
								captchaPass = false
							} else {
								if (b.r == 1) {
									captchaPass = true;
									if (isPhone($.trim($("#modal-reg-name")
											.val()))) {
										$("#pcode-area").slideDown(300);
										sendPCode("pcode-a", "modal-reg-name")
									}
...
function setSendBtStatus(a) {
	showMsg("短信验证码已经发送");
	$("#" + a).html("<span id='" + a + "-cd'>" + initSec + "</span> 秒后可以重新发送")
			.prop("disabled", true);
	if (sendCodeCounter) {
...
$(function() {
	...
	if (getCookie("cmtids") != "" && a <= 3) {
		$("#cmtidCountHref").attr("data-content", "您新建的合同都在这里.").popover()
				.focus()
	}
});
	...
	if (a.length == 0) {
		...
		$
				.post(
						...
						function(c) {
							if (c && c.r == 1) {
								var b = "<div class='comment-item'><div class='comment-avatar'><a href='#nogo' ><img src='/sitemedia/pics/avt.png'></a></div><div class='comment-body'><div class='comment-name'>"
										+ c.nick
										+ "</div><div class='comment-cont'>"
										+ c.cont
										+ "<p class='comment-tips'>(评论已成功发表，审核后将在此显示)</p></div></div></div>";
...
var queryDom = null;
$(function() {
	if ($("#search-key2").length > 0) {
		queryDom = $("#search-key2")
	} else {
...
function setQuerySug(b) {
	...
	$("#query-sug-outer").css("top", c + "px").css("left",
			queryDom.offset().left + "px").show();
...
function showMsg(b, a) {
	if (b != "") {
		if ($("#msg_area").length == 0) {
			$("body")
					.append(
							"<div id='msg_area'><span id='msg_cont'></span></div><div id='msg_mask'></div>");
...
window.onresize = function() {
	...
	if (queryDom && queryDom.length > 0) {
		$("#query-sug-outer").width(queryDom.outerWidth());
		var b = queryDom.offset().top + queryDom.outerHeight();
		$("#query-sug-outer").css("top", b + "px").css("left",
				queryDom.offset().left + "px")
	}
};
function isEmail(c) {
	var a = false;
